...
ASSETS = {filename: resources.GetResource(os.path.join(_DMC_ENVS_DIR, filename))
...
class DistractionDMCWrapper(DMCWrapper):
    ...
    def __init__(
        ...
        self._video_paths = [os.path.join(self._video_dir, f) for f in os.listdir(self._video_dir) if f.endswith('.mp4')]
