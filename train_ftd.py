...
def make_vec(env_id: str, frame_stack: int, **env_kwargs):
    ...
    env = gym.make(env_id, obs_type="pixels", channels_first=True, height=256, width=256, is_train=False, action_repeat=action_repeat, **env_kwargs)
...
def main(cfg: TrainerConfig):
    envs = gym.vector.SyncVectorEnv(
        [partial(make_vec, env_id=cfg.env_name, frame_stack=cfg.frame_stack, **cfg.env_kwargs) for _ in range(cfg.num_envs)]
    )
